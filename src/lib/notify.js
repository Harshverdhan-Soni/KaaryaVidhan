...
/**
 * Ask the browser for permission and store this device's push token against the
 * signed-in employee. Safe to call on every sign-in: it is a no-op when the
 * browser cannot do push, when the user has declined, or when messaging has not
 * been configured. Returns a short status string for the UI.
 */
export async function registerPush(empId) {
